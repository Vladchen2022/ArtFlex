...
最近一轮右侧面板 UX 也有几条已冻结的小基线，需要新线程直接沿用：`笔尖形状设计 / 导航器` 选项卡默认打开 `导航器`；`涂抹` 不再继续跟随最新普通画笔，而是第一次继承后独立记住自己上一次的 brush 设置；左侧 `参考图` tab 当前默认打开，小窗只负责固定 fit 预览与取色，放大后使用独立浮窗浏览与取色，关闭浮窗不再弹出工程保存确认；`颜色` 面板当前为了给 `画笔库` 让空间，已经把 `光色条 + 光色 / 明度 / 纯度 / 对比 / 补色` 收进底部高级区，启动时默认收起，通过底部细把手展开 / 收起。
