...
以下原规范保留为 8 位兼容规则。候选实现新增显式文档精度，不以显示格式反推文档格式：

- 标准文档继续使用 BGRA8Unorm_sRGB；高精度文档使用 RGBA16Float，线性 sRGB 原色、预乘 alpha，每像素 8 字节。
- `CanvasPixelEncoding`／`CanvasPixelCodec` 与 `LayerTextureSnapshot.encoding` 集中处理读回和转换。16 位小端 Float16 按 RGBA 存放；旧快照缺少编码时按原格式解码。
- 所有工具输入的直通道 sRGB 颜色必须先转线性再预乘。Metal `_sRGB` 目标在写入时自动编码，不可将 sRGB 数值直接当线性色输出；浮点目标不自动做这一编码。
- 图层、历史、恢复、工程保留原编码；不允许以 4 字节步长解释高精度内容。8 位来源可显式提升；16→8 只能在明确输出／转换边界进行。
- 主窗口 drawable、缩略图及视频可保持 8 位派生显示，不能由它们反写高精度图层。取色使用真实图层或统一合成资源。
- PNG／TIFF 16 位输出为标准 sRGB 整数通道及 straight alpha，浮点重采样在最终编码前进行；JPEG 为 8 位。此功能不是 HDR 或广色域支持。
- 蒙版暂保持 8 位覆盖率。详见 `Docs/PAINTING_CORE_UPGRADE_20260908.md`，该候选仍待真实屏幕验收。
