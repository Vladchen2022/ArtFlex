...
struct MetalStrokeEngineQueueTests {
    ...
    @Test
    func lightweightOilPaintIsDeterministicAndMateriallyDistinct() throws {
        guard let metalContext = MetalDeviceContext() else {
            Issue.record("Metal unavailable")
            return
        }
        ...
        let first = try renderOilPaintSnapshot(
            metalContext: metalContext,
            renderer: renderer,
            seed: 0x1357_2468,
            paintLoad: 0.18,
            includesCompanions: true
        )
        let replay = try renderOilPaintSnapshot(
            metalContext: metalContext,
            renderer: renderer,
            seed: 0x1357_2468,
            paintLoad: 0.18,
        ...
        let variant = try renderOilPaintSnapshot(
            metalContext: metalContext,
            renderer: renderer,
            seed: 0x89AB_CDEF,
            paintLoad: 0.18,
        ...
        let highLoad = try renderOilPaintSnapshot(
            metalContext: metalContext,
            renderer: renderer,
            seed: 0x1357_2468,
            paintLoad: 0.95,
        ...
        let basePigmentOnly = try renderOilPaintSnapshot(
            metalContext: metalContext,
            renderer: renderer,
            seed: 0x1357_2468,
            paintLoad: 0.18,
            includesCompanions: false
        )
        let standard = try renderPaintJitterSnapshot(
            metalContext: metalContext,
            renderer: renderer,
            seed: 0x1357_2468,
            amount: 0,
        ...
        let opacityCap = try renderOilPaintSnapshot(
            metalContext: metalContext,
            renderer: renderer,
            seed: 0x1357_2468,
            paintLoad: 0.18,
        ...
        let opacityCapReplay = try renderOilPaintSnapshot(
            metalContext: metalContext,
            renderer: renderer,
            seed: 0x1357_2468,
            paintLoad: 0.18,
        ...
        #expect(first.pixelData == replay.pixelData)
        #expect(first.pixelData != variant.pixelData)
        #expect(first.pixelData != basePigmentOnly.pixelData)
        #expect(alphaBytes(in: first) != alphaBytes(in: standard))
        #expect(alphaBytes(in: highLoad).reduce(0) { $0 + Int($1) }
            > alphaBytes(in: first).reduce(0) { $0 + Int($1) })
        #expect(opacityCap.pixelData == opacityCapReplay.pixelData)
        #expect(opacityCap.pixelData != first.pixelData)
    }
...
private func renderOilPaintSnapshot(
    metalContext: MetalDeviceContext,
    renderer: StageOneBrushRenderer,
    seed: UInt32,
    paintLoad: Float,
    includesCompanions: Bool,
    buildMode: BrushBuildMode = .buildUp
) throws -> LayerTextureSnapshot {
    let surfaceStore = StageOneLayerSurfaceStore()
    let serializer = LayerTextureSerializer(metalContext: metalContext)
    guard
    ...
    brush.buildMode = buildMode
    brush.medium = .oil
    brush.oilPaint.paintLoad = paintLoad
    brush.oilPaint.colorSeparation = 0.82
    brush.oilPaint.dryness = 0.76
    brush.oilPaint.bristleSpread = 0.72
    if includesCompanions {
        brush.oilPaint.companionColorA = .init(red: 0.96, green: 0.68, blue: 0.08, alpha: 1)
        brush.oilPaint.companionColorB = .init(red: 0.08, green: 0.28, blue: 0.82, alpha: 1)
    }
    ...
    let stroke = StrokeDescriptor(
        ...
        brush: brush,
        points: [
            .init(x: 28, y: 48, pressure: 0.35),
            .init(x: 72, y: 48, pressure: 0.55),
            .init(x: 120, y: 48, pressure: 0.75),
            .init(x: 164, y: 48, pressure: 1)
        ],
        selectionShape: nil,
        paintVariationSeed: seed
