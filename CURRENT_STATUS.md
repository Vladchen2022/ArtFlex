...
当前工作分支已完成 [绘画核心升级](Docs/PAINTING_CORE_UPGRADE_20260908.md)：增量恢复、原生稀疏图层、方案共享底稿、保留裁剪像素和 16 位浮点。已安装构建 `20260908.25`，967 项／94 组回归通过；五项代表性界面流程已验收，包含最终版 4K 两代自动恢复，原画稿已重新打开。详细结果与未覆盖边界见 [屏幕验收记录](Docs/PAINTING_CORE_SCREEN_ACCEPTANCE_20260908.md)，不要将下方既往版本结果混作本次验收。
...
- [对称局部撤销快照](Docs/REGION_UNDO_ACCEPTANCE_20260908.md)：构建 `20260908.21`，小范围操作撤销／重做仍保留原区域，避免转为整层 CPU 历史；大区域复用分块传输。两轮全量各 935 项通过，已直接验证 12 笔来回预览的画面与历史内存规模。
- [历史预览的保存与恢复保护](Docs/HISTORY_PREVIEW_SAVE_SAFETY_20260908.md)：构建 `20260908.22`，修复未确认旧历史可被保存、覆盖最新恢复副本的问题；两轮全量各 939 项通过。界面已验证预览内拒绝保存、停留期间恢复副本不变、取消后恢复继续、应用后保存重开正常。
