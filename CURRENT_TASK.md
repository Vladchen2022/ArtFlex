...
- 当前已新增 `绘画数据` 工具：左侧工具栏提供 `绘画数据` 按钮，点击后以 popover 形式显示 `总绘画时长 / 当前作品用时 / 今日绘画时长 / streak / 最近里程碑 / 过去 12 周热力图`；这条功能当前采用 `活跃创作计时 + 60 秒宽限期`，除真实笔触链 `画笔 / 橡皮 / 涂抹` 外，也已纳入 `油漆桶 / 直线 / 直线渐变 / 扇形渐变 / 套索填充 / 套索擦除 / 普通选区填充与删除 / 图形生成器 / 套索选区 / 矩形选区 / 椭圆选区`，应用失焦或窗口失去焦点时会立即暂停
- 右侧顶部 `笔尖形状设计 / 导航器` 选项卡当前默认打开 `导航器`；这条默认值当前已作为 UX 基线接受
- `涂抹` 当前不再继续跟随“最新普通画笔”的设置；其语义已改为：第一次切到涂抹时先继承当前画笔，但一旦在涂抹工具里改过参数，之后会独立记住自己上一次使用的画笔设置
- `参考图` 面板当前已定为冻结基线：左侧面板默认打开 `参考图` tab；小窗只负责固定 fit 预览与取色；放大后使用独立浮窗浏览与取色；当前浮窗关闭不应再弹出工程保存确认
- `颜色` 面板当前已做高度收口：`光色条 + 光色 / 明度 / 纯度 / 对比 / 补色` 统一并入底部高级区，软件启动时默认收起，通过底部细小把手展开 / 收起，以给下方 `画笔库` 面板让出更多空间
...
- 当前已新增确认的产品规则：
  ...
  - `绘画数据` 当前冻结为“工具栏按钮点击弹出 popover，不做常驻面板”：统计采用 `活跃创作计时 + 60 秒宽限期`，当前活动范围已扩展到 `画笔 / 橡皮 / 涂抹 / 油漆桶 / 直线 / 直线渐变 / 扇形渐变 / 套索填充 / 套索擦除 / 普通选区填充与删除 / 图形生成器 / 套索选区 / 矩形选区 / 椭圆选区`；全局统计写入 `Application Support/ArtFlex/drawing-stats.json`，当前作品时长通过文档 metadata 的 `drawingStatsID + accumulatedPaintingTime` 续接，因此“未保存先画、第一次保存后再继续画”的时间会并入同一作品
  - 启动与新建画布的当前笔刷大小默认值当前固定为 `60`，但不允许覆盖画笔库 preset 自己保存的尺寸
  - 图层面板底部按钮顺序当前冻结为“新建 / 复制 / 删除”；删除按钮保持红色并固定在第三个位置
  - 颜色面板当前默认收起高级区；收起时只保留顶部按钮、主拾色区域与底部折叠把手
