...
- 当前任务已切换为：填充 / 渐变工具、快照 / 方案试探 / 绘画数据与默认 UX 基线收口；Dual Tip 当前冻结，除非有明确新需求或实际 bug，不再主动改动
...
- 当前已新增 `绘画数据` 工具：左侧工具栏提供 `绘画数据` 按钮，点击后以 popover 形式显示 `总绘画时长 / 当前作品用时 / 今日绘画时长 / streak / 最近里程碑 / 过去 12 周热力图`；这条功能当前采用 `活跃笔触计时 + 60 秒宽限期`，只统计真实笔触链 `画笔 / 橡皮 / 涂抹`，应用失焦或窗口失去焦点时会立即暂停
...
- 当前已新增快捷 HUD 拾色器：按住 `Shift + Z` 会在笔尖附近弹出轻量拾色器，松开消失；HUD 当前会同步右侧主拾色器的主要滑块语义，并在下方映射画笔库前四格
- 启动与新建画布时，当前笔刷大小默认值已改为 `60`；这只影响当前 `toolSession` 默认值，不改画笔库 preset 自己保存的 size
...
- 当前已新增确认的产品规则：
  ...
  - `锁定透明像素` 当前定义为“锁定当前图层已有非透明像素范围”，不是图层对图层的剪贴蒙版
  - 快捷 HUD 拾色器当前冻结为 `Shift + Z`：按住弹出、松开消失，默认以当前光标为中心显示，靠边时再做最小避让
  - `绘画数据` 当前冻结为“工具栏按钮点击弹出 popover，不做常驻面板”：统计采用 `活跃笔触计时 + 60 秒宽限期`，只统计 `画笔 / 橡皮 / 涂抹` 的真实笔触；全局统计写入 `Application Support/ArtFlex/drawing-stats.json`，当前作品时长通过文档 metadata 的 `drawingStatsID + accumulatedPaintingTime` 续接，因此“未保存先画、第一次保存后再继续画”的时间会并入同一作品
  - 启动与新建画布的当前笔刷大小默认值当前固定为 `60`，但不允许覆盖画笔库 preset 自己保存的尺寸
