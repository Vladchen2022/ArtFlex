...
### 0.18 绘画数据当前冻结为“工具栏按钮弹出 popover + 活跃创作计时”

已确认：

- `绘画数据` 当前不做常驻面板，正确入口是左侧工具栏按钮点击后弹出 popover
- 面板当前显示：
  - `总绘画时长`
  - `当前作品用时`
  - `今日绘画时长`
...
- 计时策略当前冻结为：
  - 当前活动范围已扩展为：
    - 真实笔触链 `画笔 / 橡皮 / 涂抹`
    - 像素类单次操作 `油漆桶 / 直线 / 直线渐变 / 扇形渐变 / 套索填充 / 套索擦除 / 普通选区填充与删除 / 图形生成器`
    - 选区提交操作 `套索选区 / 矩形选区 / 椭圆选区`
  - 创作操作结束后保留 `60 秒` 宽限期
  - 切图层、缩放、平移、拖面板等非创作操作不计时
  - 应用进入后台或窗口失去焦点时立即暂停，不再继续吃宽限期
- `绘画数据` 当前必须继续避开渲染热路径：
  - 不读纹理
  - 不做快照
  - 不碰 Metal 主绘制链
- 全局统计当前写入 `Application Support/ArtFlex/drawing-stats.json`
- 当前作品时长当前写入文档 metadata：
  - `drawingStatsID`
  - `accumulatedPaintingTime`
- 这意味着“未保存先画、第一次保存后再继续画”的时间必须并入同一作品，不允许第一次保存时把未保存阶段的绘画时间丢掉
- `方案试探` 分支当前必须共享同一个 drawing stats service，不允许因为同步推进或镜像操作把同一段创作时间重复记多次

### 0.19 右侧顶部默认 tab、涂抹记忆、参考图与颜色面板 UX 基线已冻结

已确认：

- 右侧顶部 `笔尖形状设计 / 导航器` 选项卡默认打开 `导航器`
- `涂抹` 不再继续跟随最新普通画笔；第一次切到涂抹时先继承当前画笔，但一旦在涂抹里改过参数，之后必须独立记住自己上一次的 brush 设置
- 左侧 `参考图` 面板默认打开 `参考图` tab
- 参考图小窗只负责固定 fit 预览与取色
- 参考图放大后使用独立浮窗浏览与取色
- 关闭参考图浮窗不再弹出工程保存确认
- `颜色` 面板已将 `光色条 + 光色 / 明度 / 纯度 / 对比 / 补色` 收进底部高级区
- 颜色高级区在软件启动时默认收起，通过底部细把手展开 / 收起
