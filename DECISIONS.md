...
### 0.13 `直线渐变 / 扇形渐变` 当前冻结为“停用待重建”

已确认：

- `直线渐变` 与 `扇形渐变` 当前不再作为稳定功能继续维护
- 为避免牵连现有工具、工程状态和快捷键体系，当前保留工具枚举做兼容，但 UI 入口、快捷键循环和工具切换主链已统一回退到 `油漆桶`
- 后续若要恢复这两个工具，默认按“重建”处理，而不是继续在当前旧实现上打补丁
